...
class AppStore extends Component {
  state = {isActiveTab: tabsList[0].tabId, searchInput: ''}
  ...
  getFilteredProjects = searchedApps => {
    ...
    const filteredProjects = searchedApps.filter(
      eachApp => eachApp.category === isActiveTab,
    )
  ...
  render() {
    ...
    const filteredProjects = this.getFilteredProjects(searchResults)
    ...
    return (
      <div className="bg-con">
        ...
        <ul className="main-items">
          {filteredProjects.map(eachObject => (
            <AppItem key={eachObject.appId} eachObject={eachObject} />
          ))}
        </ul>
      </div>
    )
